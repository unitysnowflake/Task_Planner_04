workspace {
    ...
    model {
        ...
        ss = softwareSystem "Task Planner" "Система для управления целями и задачами" {
            ...
            userService = container "User Service" "Управление пользователями (CRUD) и аутентификация" "Python (FastAPI)" {
    ...
    views {
        ...
        container ss {
            include *
            autolayout lr
            description "Контейнер Task Planner, показывающий основные сервисы, данные пользователей (PostgreSQL) и данные целей и задач (MongoDB)."
        }
